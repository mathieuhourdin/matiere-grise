<template>
  <div class="md:p-6 mx-auto">
    ...
    <div class="mb-6">
      <label
        class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3 cursor-pointer select-none flex items-center gap-2"
        @click="!isRecording && canRecord && !isSubmitting ? toggleRecording() : isRecording && canRecord && !isSubmitting ? toggleRecording() : null"
        :class="[
          (!canRecord || isSubmitting) && 'opacity-50 cursor-not-allowed',
          isRecording
            ? 'text-red-600 dark:text-red-400 animate-pulse'
            : 'hover:text-blue-500 dark:hover:text-blue-400 transition-colors duration-150'
        ]"
        :title="isRecording ? 'Arrêter l\'enregistrement' : (!canRecord ? 'Enregistrement indisponible' : (isSubmitting ? 'Veuillez patienter' : 'Commencer l\'enregistrement'))"
      >
        <span>
      ...
      <div class="flex flex-col items-center space-y-4">
        ...
        <div v-if="audioUrl && !isRecording" class="w-full">
          <div class="bg-gray-50 dark:bg-elevated rounded-lg p-4 border border-gray-200 dark:border-gray-700">
...
const { context } = useQuickCreateModal()
...
const submitTextOnly = async () => {
  const trace = { content: textContent.value, journal_id: context.value }
  ...
  setTimeout(() => {
    ...
    router.push(`/app/resources/${createdTrace.id}`)
  }, 1500)
}
</script>
