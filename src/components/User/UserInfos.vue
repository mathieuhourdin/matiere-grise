<template>
  <div class="text-center p-4 bg-slate-200">
    ...
    <div>{{ user.handle }}</div>
    <div>{{ user.email }}</div>
    <RoundLinkButton class="right-4 absolute" v-if="isCurrentUser" @click="editUser = true"
      ><PencilSquareIcon class="m-1"
    /></RoundLinkButton>
    </div>
    <div v-else>
      <TextInput class="mx-auto w-2/3" v-model="editableUser.profile_picture_url" label="Photo de profil" />
...
const isCurrentUser = computed(() => {
  return props.user.id === authUser.value.id
})
...
onMounted(() => editableUser.value = props.user)

watch(editableUser, (editableUser) => debouncedUpdateUser(editableUser.id, editableUser), { deep: true})
</script>
