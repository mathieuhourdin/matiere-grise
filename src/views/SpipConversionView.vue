<template>
  <div class="flex flex-col p-4">
    <div class="text-sm mb-2">
      Conversion au format SPIP : ajouter le contenu à partir d'un fichier .docx (.doc ne marche pas) puis copier (icon en haut à droite) coller
      dans spip.
      Le gras est mis en gras, et les parties entre guillements sont mises en italique. Attention aux insertions de discours indirect dans les citations, qui doivent être remises en non italique.
    </div>
    <input
      id="file"
      type="file"
...
const uploadFile = async (event) => {
  ...
  const response = await postFileConversion(formData, 'spip')
