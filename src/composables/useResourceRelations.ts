...
const getResourceRelationsForResource = async (resource_id: string) => {
  const response = await fetchWrapper.get('/resources/' + resource_id + '/bibliography')
  return response.data
}
const getUsagesForResource = async (resource_id: string) => {
  const response = await fetchWrapper.get('/resources/' + resource_id + '/usages')
  return response.data
}

const getAllRelationsForResource = async (resource_id: string) => {
  const responseBiblio = await fetchWrapper.get('/resources/' + resource_id + '/bibliography')
  const responseUsages = await fetchWrapper.get('/resources/' + resource_id + '/usages')
